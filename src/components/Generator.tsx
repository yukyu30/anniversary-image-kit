...
const FRAME_OPTIONS = siteConfig.frames;

const FRAME_PNG: Record<string, string> = Object.fromEntries(
  siteConfig.frames.map((f) => [f.value, f.src]),
);
...
export function Generator() {
  ...
  const [frameColor, setFrameColor] = useState<FrameColor>(
    siteConfig.frames[0].value,
  );
  // フレーム画像の縦横比は読み込んで動的に判定
  const [frameAspect, setFrameAspect] = useState(4.4);
  ...
  useEffect(() => {
    const img = new Image();
    img.onload = () => setFrameAspect(img.width / img.height || 4.4);
    img.src = FRAME_PNG[frameColor];
  }, [frameColor]);
  ...
  function shareToX() {
    const text = `${siteConfig.avatar.nameJa}${siteConfig.anniversary.labelJa}記念の画像を作りました！\n${location.origin}\n${siteConfig.hashtag}`;
    window.open(
  ...
  // フレーム（リボン）の写真内での表示位置（%）
  const heightFrac = (frame.size * photoAspect) / frameAspect;
  const framePct = {
