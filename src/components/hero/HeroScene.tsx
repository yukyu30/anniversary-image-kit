...
/** 紙吹雪に紛れて時々落ちてくる墨澄ピース */
function SumiPieces() {
  const texture = useTexture(siteConfig.hero.fallingPiece);
...
/** 立ち絵（透過テクスチャの板） */
function Character() {
  const texture = useTexture(siteConfig.hero.standingImage);
...
function AnnivText() {
  ...
  useLayoutEffect(() => {
    ...
    const draw = () => {
      ...
      // アバター名
      const nameJa = siteConfig.avatar.nameJa;
      const label = siteConfig.anniversary.labelJa;
      ctx.font = `400 ${fit(nameJa, 340)}px ${family}`;
      ctx.fillText(nameJa, W / 2, 320);
      // 周年ラベル
      ctx.font = `400 ${fit(label, 500)}px ${family}`;
      ctx.fillText(label, W / 2, 760);
    ...
    if (fonts?.load) {
      fonts
        .load(
          `400 460px ${family}`,
          `${siteConfig.avatar.nameJa}${siteConfig.anniversary.labelJa}`,
        )
        .then(draw)
        .catch(draw);
    } else {
